...
// Define a command configuration object
const commandConfig = {
  pattern: "wamod",
  alias: ["modwhatsapp"],
  react: '🪀',
  desc: "Download Mod WhatsApp",
  category: "download",
  use: ".wamod",
  filename: __filename,
};
...
cmd(commandConfig, async (context, options, extra, {
  from,
  quoted,
  body,
  isCmd,
  ...
  try {
    // Fetch mod download data from a remote source
    const modData = (await fetchJson("https://vihangayt.me/download/fmmods")).data;

    // Define buttons for different WhatsApp mods
    const buttons = [
      {
        buttonId: `${global.PREFIX}dmod ${modData.com_whatsapp.link}+${modData.com_whatsapp.name}`,
        buttonText: { displayText: "WhatsApp" },
        type: 1,
      },
      {
        buttonId: `${global.PREFIX}dmod ${modData.com_fmwhatsapp.link}+${modData.com_fmwhatsapp.name}`,
        buttonText: { displayText: "FM WhatsApp" },
        type: 1,
      },
      {
        buttonId: `${global.PREFIX}dmod ${modData.com_gbwhatsapp.link}+${modData.com_gbwhatsapp.name}`,
        buttonText: { displayText: "GB WhatsApp" },
        type: 1,
      },
      {
        buttonId: `${global.PREFIX}dmod ${modData.com_yowhatsapp.link}+${modData.com_yowhatsapp.name}`,
        buttonText: { displayText: "YO WhatsApp" },
        type: 1,
      },
    ];

    // Image and message content
    const image = { url: "https://telegra.ph/file/576e4f11b3e2d7bed7318.jpg" };
    const messageContent = {
      image,
      caption: "*🪀 WhatsApp Mods By Queen Nilu*",
    ...
    return await context.buttonMessage2(from, messageContent, options, options);
  } catch (error) {
    // Handle errors
